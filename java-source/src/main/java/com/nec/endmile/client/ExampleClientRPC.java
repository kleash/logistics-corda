package com.nec.endmile.client;
...
public class ExampleClientRPC {
    ...
    private static void logState(StateAndRef<CourierState> state) {
    ...
    public static void main(String[] args) throws ActiveMQException, InterruptedException, ExecutionException {
        ...
        final DataFeed<Vault.Page<CourierState>, Vault.Update<CourierState>> dataFeed = proxy.vaultTrack(CourierState.class);
        final Vault.Page<CourierState> snapshot = dataFeed.getSnapshot();
        final Observable<Vault.Update<CourierState>> updates = dataFeed.getUpdates();
